...
def normalize(column):
    """
    Function to z-score normalize a feature vector
    :param column: feature vector
    :return: z-score normalized feature vector
    """
    # Convert raw_data to a NumPy array
    feature_vector = np.array(column, dtype=float)

    mean_v = np.mean(feature_vector)  # Get mean of the feature vector
    std_v = np.std(feature_vector)    # Get standard deviation of the feature vector
    ...
    normalized_vector = (feature_vector - mean_v) / std_v

    return normalized_vector

class RootData:
    def __init__(self, path, is_class=True, print_folds=False):
        """
        Class used to read data in from file, performing processing, and create folds

        :param path: string path to a data file
        :param is_class: Boolean denoting whether it is a classification or regression problem
        """
    ...
    def load(self):
        ...
        self.hyperparameters['learning_rate'] = Hyperparameter('learning_rate', 0.000001, 0.1)
        self.hyperparameters['batch_size'] = Hyperparameter('batch_size', 0, 0) # TODO: find min and max value for batch size
